...
#   ) %>%
#   ungroup() %>%
#   mutate(fpts = (yards/10 + TD*6) - fumbles*2) %>%
...
weekly_stats <- advanced_receiving_stats %>%
  mutate(
    `TGT SHARE` = paste0(format(
      100 * target_share, digits = 1, nsmall = 1
    ), "%"),
    `AIR SHARE` = paste0(format(
      100 * air_yards_share, digits = 1, nsmall = 1
    ), "%"),
    PTS = format(fpts, digits = 1, nsmall = 1)
  ) %>%
  select(
    Player = receiver,
    WEEK = week,
...
table <-
  reactable(
    ...
    details = function(index) {
      ...
      htmltools::div(style = list(margin = "12px 45px"), weekly_stats_tbl)
    },
    onClick = "expand",
